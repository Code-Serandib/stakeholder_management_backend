...
function signupParameterizedQuery(Users users) returns sql:ParameterizedQuery {
    ...
    // Generate access token (You can implement a token generation logic here)
    string accessToken = "generated_access_token"; // Replace this with actual token generation logic

    sql:ParameterizedQuery query = `INSERT INTO users 
        (organizationName, organizationType, industry, address, country, administratorName, email, contactNumber, role, username, password, access_token) VALUES 
        (${organizationName}, ${organizationType}, ${industry}, ${address}, ${country}, ${administratorName}, ${email}, ${contactNumber}, ${role}, ${username}, ${password}, ${accessToken})`;
    return  query;
};
